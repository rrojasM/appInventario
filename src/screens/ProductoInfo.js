    ...
    <View style={{ width: '100%', height: '100%', backgroundColor: COLORS.WHITE, position: 'relative' }}>
      ...
      <ScrollView>
        ...
        <View style={{ paddingHorizontal: 16, marginTop: 6 }}>
          ...
          <View style={{ flexDirection: 'row', marginVertical: 4, alignItems: 'center', justifyContent: 'space-between' }}>
            <Text style={{ fontSize: 24, fontWeight: '600', letterSpacing: 0.5, marginVertical: 4, color: '#000', maxWidth: '84%' }}>{product.productName}</Text>
          </View>
          <Text style={{ fontSize: 12, color: '#000', fontWeight: '400', letterSpacing: 1, opacity: 0.5, lineHeight: 20, maxWidth: '85%', maxHeight: 44, marginBottom: 18 }}>{product.description}</Text>
          ...
          <View style={{ paddingHorizontal: 16, }}>
            <Text style={{ fontSize: 18, fontWeight: '500', maxWidth: '85%', color: '#000', marginBottom: 4 }}>${product.price}</Text>
          </View>
        </View>
      </ScrollView>
      <View style={{ position: 'absolute', bottom: 10, height: '8%', width: '100%', justifyContent: 'center', alignItems: 'center' }}>
        <TouchableOpacity onPress={() => (product.isAvailable? addToCard(product.id): null)} style={{ width: '86%', height: '80%', backgroundColor: COLORS.GREEN, borderRadius: 20, justifyContent: 'center', alignItems: 'center' }}>
          <Text style={{ fontSize: 12, fontWeight: '500', letterSpacing: 1, color: '#FFF', textTransform: 'uppercase' }}>{product.isAvailable ? "Agregado al Carrito" : "No disponible"}</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
